...
public class EmbeddedServiceImpl implements EmbeddedService {
    ...
    public  JSONObject voiceUpload(String taskdir,String path,String fileName) throws IOException {
        // 1. 读取并编码图像文件为 Base64
        String base64Audio = encodeImageToBase64(path+"/"+fileName);
        // 2. 构造请求体（JSON 格式）
        ...
        try (CloseableHttpClient httpClient = HttpClients.createDefault()) {
            ...
            try (CloseableHttpResponse response = httpClient.execute(post)) {
                if (response.getStatusLine().getStatusCode() == 200) {
                    ...
                    convertBase64ToWav(base64AudioDataAll.toString(), path+"/output_audio.wav", 24000);

                    Files.write(Paths.get(path+"/output_audio.txt"), base64AudioDataAll.getBytes(StandardCharsets.UTF_8));
